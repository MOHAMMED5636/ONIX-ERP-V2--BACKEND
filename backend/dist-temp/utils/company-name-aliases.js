...
exports.buildCompanyNameAliases = buildCompanyNameAliases;
exports.buildCompanyScopeAliases = buildCompanyScopeAliases;
/**
 ...
 */
function buildCompanyNameAliases(rawName) {
    const base = String(rawName ?? '').trim();
    if (!base)
        return [];
    const collapseSpaces = (s) => s.replace(/\s+/g, ' ').trim();
    const normalizeHyphenTight = (s) => s.replace(/\s*-\s*/g, '-').trim();
    const normalizeHyphenSpaced = (s) => s.replace(/\s*-\s*/g, ' - ').trim();
 ...
 */
function buildCompanyScopeAliases(companyName, parentCompanyName) {
    ...
    const baseFromDashSuffix = (() => {
        ...
        if (!raw)
            return null;
        if (!/\bONIX ENGINEERING CONSULTANCY\b/i.test(raw))
            return null;
        if (!/\bDUBAI\b/i.test(raw))
            return null;
        if (/\bBRANCH OF\b/i.test(raw))
            return null;
...
//# sourceMappingURL=company-name-aliases.js.map